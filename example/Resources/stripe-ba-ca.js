...
function stripeResponseHandler(response, status) { // Heads up: stripe documentation indicates that these callback functions receive (status, response), however, in their own codebase it actually returns (response, status). I've chosen to match their codebase functionality and not their documention in this instance
	...
	var baCARouting = Titanium.UI.createTextField({
	    keyboardType:Titanium.UI.KEYBOARD_NUMBER_PAD,
	    baStyleNum: true, // <== keeps it from formatting if set to false *NOTE* stripe still validates with the dash in this format: 12345-678, you you'll need to set baVerify to false, and validate yourself on baOnBlur
	    baVerify: true,
	    baOnFailVerify: function(tf) {
	    	this.color = 'red';
	    },
	    baOnBlur: function() {
	    	//if doing custom validation or something. It's optional
	    	//returning false will keep it from blurring during autofocus
	    },
	    autocorrect:false,
	    top: 0,
	    height:40,
		...
		left: 10,
		selectionStyle:'none',
		hintText: 'Enter Routing Number (CA)'
	});
	TiStripe.paymentKitUI.baCARoutingTextField(baCARouting); //canadian routing number
	//TiStripe.paymentKitUI.baUSRoutingTextField(baCARouting); //us routing number
	row.add(baCARouting); 


	var baCAAccount = Titanium.UI.createTextField({
	    keyboardType:Titanium.UI.KEYBOARD_NUMBER_PAD,
	    //baNextFocus: false, // <== keep it from autofocusing
	    autocorrect:false,
	    top: 40,
	    left: 10,
	    height:40,
	...
	//TiStripe.paymentKitUI.baUSAccountTextField(baCAAccount); //us account number
	row.add(baCAAccount);
	...
	payBtn.addEventListener('click', function() {
		...
		TiStripe.bankAccount.createToken({
		    country: 'CA', // or 'US' for US Accounts
		    routingNumber: baCARouting.value,
		    accountNumber: baCAAccount.value,
